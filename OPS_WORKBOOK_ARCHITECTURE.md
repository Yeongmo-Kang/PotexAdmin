...
- 唯一の ID 発行地点
- 唯一の canonical 確定地点
- CS / Executive / Concierge / Sales / Coaches / Sato / Inai 向け publish source を提供する
...
### 2.7 `Potex Sato` / `Potex Inai`
役割:
- partner ごとの担当 lead 進捗確認と status writeback 入力
...
運用ルール:
- `パートナー_担当リード` / `パートナー_データ状況` は読み取り専用
- `パートナー_状況入力` だけが partner update 用 input タブ
- assignment 自体の決定は `Potex CS > CS_担当割当入力` で行う
...
- Sato / Inai: partner update のための scoped workbook
