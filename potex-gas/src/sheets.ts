...
const LEGACY_VIEW_NAME_ALIASES: Record<string, string[]> = {
  ...
  [VIEWS.EXEC_MEETING_CHECK]: ['Exec_Meeting_Check'],
  [VIEWS.EXEC_UPDATE_STATUS]: ['Exec_Update_Status'],
...
const DISPLAY_HEADER_SHEETS = new Set<string>([
  ...
  VIEWS.EXEC_README,
  VIEWS.EXEC_MEETING_CHECK,
  VIEWS.EXEC_UPDATE_STATUS,
...
const GENERIC_DISPLAY_LABELS: Record<string, string> = {
  ...
  last_publish_success_at_jst: '直近publish成功日時',
  last_full_refresh_success_at_jst: '直近full refresh成功日時',
  last_effective_update_at_jst: '最終有効更新日時',
  expected_cadence: '想定更新頻度',
  stale_threshold: '警戒しきい値',
  stale_by: '超過状況',
  likely_issue_type: '想定される原因',
  likely_decision_risk: '会議での見誤りリスク',
  recommended_check: '優先確認先',
  stale_domain_count: '停滞ドメイン数',
  stale_high_risk_domain_count: '高リスク停滞ドメイン数',
  domains_with_likely_human_update_omission: '更新漏れ疑いドメイン',
  likely_human_update_omission_count: '更新漏れ疑い件数',
  meeting_risk_status: '会議判断ステータス',
  critical_team_issue_count: '重要案件件数',
  stale_30d: '30日以上更新なし',
