...
- 次の焦点は「運用自動化の詰まり解消」と「ERP 化しやすいモジュール分割」の両立

---

## 今やること（優先順）

### 1. `publish/views.ts` を feature 単位で分割する
- 目的:
  - ERP 的モジュール境界を壊しにくくする
  - CS / Executive / Sales / Coach / Concierge / Partner ごとに責務を分ける
- まず分ける先:
  - `publish/views/cs.ts`
...
- 完了条件:
  ...
  - live workbook の主要タブ表示に意図しない崩れがない

### 2. writeback input contract を feature ごとに整理する
- 目的:
  - 表示ラベル変更や列順変更で writeback が壊れにくい形にする
- 対象:
  - `CS_別名解決入力`
  - `CS_入金名寄せ確認`
  - `CS_継続名寄せ確認`
...
- 完了条件:
  - editable 列 / 非editable 列の境界が文書とコードで一致する
  - operator UX を崩さず内部契約を安定化できる

### 3. approval queue の運用自動化・可観測性を改善する
- 背景:
  - 現状のボトルネックは、コード未実装よりも approval queue の人手処理
- 進めること:
...
### 4. ERP module map と Customers cutover 前提を文書化する
- 目的:
