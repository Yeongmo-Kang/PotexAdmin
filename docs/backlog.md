...
원칙:
- GitHub repo `origin` (`Yeongmo-Kang/PotexAdmin`, branch `main`)를 문서/코드의 1차 기준으로 사용.
...
### P-014. Executive data trust / freshness / missing-update visibility
- plan: `docs/plans/2026-05-24-executive-data-trust-and-freshness-plan.md`
- 목적: `Potex Executive`가 단순 KPI 표시를 넘어서, **숫자를 믿어도 되는지 / 어느 도메인이 stale인지 / 사람 업데이트 누락 가능성이 어디인지**를 회의 전에 바로 보여주게 한다.
- 핵심 요구:
  - `経営_会議前チェック` 탭 추가
  - `経営_更新状況` 탭 추가
  - `経営_データ状況`에 freshness / stale-domain / likely human-update-omission metric 추가
  - stale 상태일 때 단순 경고가 아니라 `의사결정이 어떤 방향으로 bias될 수 있는지`와 `어디를 먼저 확인해야 하는지`까지 표시
- 구현 원칙: 레거시 workbook의 README-first / role-first / Japanese UX는 유지하고, 계산/신뢰 판단은 Apps Script publish 시점에 수행한다. Executive workbook에는 복잡한 formula web을 추가하지 않는다.
