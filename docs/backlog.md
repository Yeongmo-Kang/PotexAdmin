...
업데이트: 2026-05-26T17:30:00+09:00
...
### Phase A — customer v2 pipeline (P-014)
- **목표:** LStep csvD → `POTEX DB.Line_Registrations` / `ConversionHistory` → `Customers.lifecycle_status` → `POTEX_顧客管理_v2.顧客一覧_成約以降` 운영자 뷰.
- **운영자 피드백 5개 처리 결과 (2026-05-26):**
  - ✅ ① `状態` 컬럼 (`受講者管理`에 단일자 헤더, blank/`クーオフ`/`返金`/`解約`) → `Customers.lifecycle_status` sync → `顧客一覧_成約以降` 회색 grayout.
  - ✅ ② R 컬럼 `直近入金日` — canonical `Payments` (legacy `入金管理`/`着金管理マスター` 데이터 소스) → publish 시점 derive. 이미 구현됨.
  - ⏸ ③ J:L 컬럼 deprecation — 운영자 회답 "일단 나중에", 보류.
  - ✅ ④ AFS 진행 통합 — `AFSプラン` / `AFS契約日` 2 컬럼을 `今後提案可能性` 뒤에 추가 (R 컬럼 위치 보존).
  - ✅ ⑤ テンプレ 탭 함수화 — 운영자 회답 "이미 되어있음".
- **commits push 완료 (GitHub origin/main):** `e6f00dd` lifecycle_status / `00555a3` AFS columns / `31f678f` Import_csvD auto-create.
- **clasp push pending:** 양쪽 환경 token 만료 (`invalid_grant`). 운영자가 v2 메뉴 클릭 전 OAuth 재인증 1회 필요. 재인증 후 dist/code.js 반영하면 메뉴 즉시 사용 가능.
- **runWritebackCustomerV2는 Phase A 스텁** — v2 운영자 편집을 DB로 역류시키는 input 탭은 Phase B에서 설계.
